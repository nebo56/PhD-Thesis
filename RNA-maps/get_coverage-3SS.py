...
description:
Script will accept fasta sequnces of flanked exonic 3' splice site positions by 300 nt upstream and 300 downstream.
...
def get_coverage(fin_fname, fin_fname_motifs, fout_fname):
    ...
    while line:
        if line[0] == '>':
            ...
            if cluster_start < -600: cluster_start = -600     #if cluster is outside of our map region
            if cluster_end < -600:   cluster_end = -600
            #if original_cluster_end > 600:   original_cluster_end = 600
            ...
            fout.write(original_exon + ',' + str(info_col[0]) + ',' + str(info_col[1]) + "," + str(total_coverage).replace('[','').replace(']','') + '\n')
