...
data=$1
#path=`pwd -P`
path=/cluster/project9/ule-group/irCLIP/
...
python ${path}scripts/swap_barcode_to_header-irCLIP.py ${path}${data}-incomplete.fa ${path}${data}-incomplete-barcodes.fa
python ${path}scripts/swap_barcode_to_header-irCLIP.py ${path}${data}-complete.fa ${path}${data}-complete-barcodes.fa
...
python ${path}scripts/BEDtoXlink.py ${path}${data}-all-uniq-2mis.bed ${path}${data}-all-uniq-2mis-xlink.bed
sort -k1,1 -k2,2n -k6,6 ${path}${data}-all-uniq-2mis-xlink.bed > ${path}${data}-all-uniq-2mis-xlink-sorted.bed
python ${path}scripts/BEDsum.py ${path}${data}-all-uniq-2mis-xlink-sorted.bed ${path}${data}-all-uniq-2mis-xlink-sorted-sum.bed
python ${path}scripts/BED2BEDgraph.py ${path}${data}-all-uniq-2mis-xlink-sorted-sum.bed ${path}${data}-all-uniq-2mis-xlink-sorted-sum.bedGraph
...
python ${path}scripts/separate_cDNAs_40nt_length.py ${path}${data}-complete-uniq-2mis.bed ${path}${data}-complete-uniq-2mis-40less.bed
...
python ${path}scripts/count_cDNAs.py ${path}${data}-all-uniq-2mis-xlink-sorted-sum.bed >> ${path}${data}-01.Mapping-REPORT.txt
...
python ${path}scripts/flankBEDpositionsCustom.py ${path}${data}-all-uniq-2mis-xlink-sorted.bed ${path}${data}-all-uniq-2mis-xlink-sorted-flanked2.bed 2 2
...
python ${path}scripts/kmer_finder.py ${path}${data}-all-uniq-2mis-xlink-sorted-flanked2.fasta ${path}${data}-xlinks-flanked2-kmers.txt
...
python ${path}scripts/narrow_cDNA_length_distribution.py ${path}${data}-all-uniq-2mis.bed >> ${path}${data}-01.Mapping-REPORT.txt
